...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // 赋值语句返回值为右侧所赋予的值
        if ((tab = table) == null || (n = tab.length) == 0)
            // 如果 table 为空，或者 table 的长度为 0，先执行一次 resize
            n = (tab = resize()).length;
        if ((p = tab[i = (n - 1) & hash]) == null)
            // 如果经过 hash 得到的 index 对应数组没有存放任何 Node，直接创建新的 Node，next 指向 null
            // p 为经过 hash 值找到的 array 中的链表的头
            ...
            if (p.hash == hash &&
                ...
                // 如果带存的 key hash 值和 p hash 值一样，同时 key 也一样，那么 e 设为 p 值，再强调一下
                // p 是通过 hash 找到的 链表的第一个 Node （如果是 null，前面的 if 已经处理）
                // e 可以是 existing，也就是已经存在的值
                e = p;
            else if (p instanceof TreeNode)
                // 如果 p 是一个 tree，那么就要用树的方式存储该值
            ...
            // 如果 e 不为空，表示新插入的值的 key 已经存在。
            if (e != null) { // existing mapping for key
                V oldValue = e.value;
                if (!onlyIfAbsent || oldValue == null)
                    // 如果 原来的值为 null 或者 onlyIfAbsent 为 false（也就是需要改变已存在的值）
                    // 将 已经存在的值覆盖为新的值，同时返回旧值
                    e.value = value;
                // TODO: Callback?
                afterNodeAccess(e);
                return oldValue;
        ...
        ++modCount;
        if (++size > threshold)
            // 如果 整体的  size 超过了 threshold，那么需要重新调整
            resize();
    ...
    final Node<K,V>[] resize() {
        ...
        // 如果 oldTab 不是 null（不是第一次），那么 oldCap 就是 oldTab 的长度
        int oldCap = (oldTab == null) ? 0 : oldTab.length;
        // 旧的 threshold
        int oldThr = threshold;
        int newCap, newThr = 0;
        // 如果不是第一次 resize（），因为只要不是第一次扩容，就的容量都会大于0
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) {
                // 如果 oldCap 已经超过 MAXIMUM_CAPACITY， 将 threshold 设为无限大
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                     oldCap >= DEFAULT_INITIAL_CAPACITY)
                // 否则 oldCap 2次方 是否小于 MAXIMUM_CAPACITY，并且 oldCap 应该大约默认初始容量，则
                // 新的阈值为 旧的阈值的 2倍
                newThr = oldThr << 1; // double threshold
        }
        else if (oldThr > 0) // initial capacity was placed in threshold
            // 如果 就得 threshold 不是0： 第一次用户指定了 threshold， 或者不是第一次
            newCap = oldThr;
        else {               // zero initial threshold signifies using defaults
            // 默认情况下第一次 扩容， 16 个array
            // threshold 为 16* factor
            newCap = DEFAULT_INITIAL_CAPACITY;
            // factor 在容量设定好后，就被使用并计算出 threshold，在实际大小变化时，直接和 threshold 比较即可
            // 参见 putValue 函数 判断是否 resize() 的 if 语句。
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);
        }
        // 如果用户指定了 threshold，那么就要根据 这个 threshold 来计算新的 threshold
        if (newThr == 0) {
